...
class TestBanditScanner:
    ...
    @patch("tasks.c")
    def test_bandit_success_with_valid_json(self, mock_c):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            valid_output = json.dumps(
                {"results": [{"issue_severity": "HIGH", "issue_confidence": "HIGH"}], "metrics": {"_totals": {"HIGH": 1}}}
            )
    ...
    @patch("tasks.c")
    def test_bandit_fails_when_output_file_missing(self, mock_c):
    ...
    @patch("tasks.c")
    def test_bandit_fails_on_empty_output(self, mock_c):
    ...
    @patch("tasks.c")
    def test_bandit_fails_on_invalid_json(self, mock_c):
...
class TestTruffleHogScanner:
    ...
    @patch("tasks.c")
    def test_trufflehog_success_with_valid_output(self, mock_c):
    ...
    @patch("tasks.c")
    def test_trufflehog_fails_when_output_file_missing(self, mock_c):
    ...
    @patch("tasks.c")
    def test_trufflehog_fails_on_empty_output(self, mock_c):
...
class TestCheckovScanner:
    ...
    @patch("tasks.c")
    def test_checkov_success_with_valid_json(self, mock_c):
    ...
    @patch("tasks.c")
    def test_checkov_fails_when_output_file_missing(self, mock_c):
    ...
    @patch("tasks.c")
    def test_checkov_fails_on_empty_output(self, mock_c):
    ...
    @patch("tasks.c")
    def test_checkov_fails_on_invalid_json(self, mock_c):
...
class TestScanAllTask:
    ...
    def test_scan_all_succeeds_when_all_scanners_pass(self, mock_bandit, mock_truffleHog, mock_checkov):
    ...
    @patch("tasks.scan_trufflehog")
    @patch("tasks.scan_bandit")
    def test_scan_fails_when_bandit_fails(self, mock_bandit, mock_truffleHog, mock_checkov):
        ...
        with pytest.raises(RuntimeError, match="1 scanner\\(s\\) failed"):
            scan(mock_c)

    @patch("tasks.scan_checkov")
    @patch("tasks.scan_trufflehog")
    @patch("tasks.scan_bandit")
    def test_scan_fails_when_multiple_scanners_fail(self, mock_bandit, mock_truffleHog, mock_checkov):
        """scan task fails if multiple scanners fail."""
        from tasks import scan

        mock_c = MagicMock()
        mock_bandit.side_effect = RuntimeError("Bandit output empty")
        mock_truffleHog.side_effect = RuntimeError("TruffleHog output empty")

        with pytest.raises(RuntimeError, match="2 scanner\\(s\\) failed"):
            scan(mock_c)

    @patch("tasks.scan_checkov")
    @patch("tasks.scan_trufflehog")
    @patch("tasks.scan_bandit")
    def test_scan_continues_after_scanner_failure(self, mock_bandit, mock_truffleHog, mock_checkov):
