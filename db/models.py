...
class Trade(Base):
    ...
    rsi_at_signal: Mapped[float | None] = mapped_column(Float, nullable=True)
...
class PortfolioSnapshot(Base):
    ...
    portfolio_value: Mapped[float | None] = mapped_column(Float, nullable=True)  # positions value
...
class BotStatus(Base):
    ...
    token_saved_at: Mapped[datetime | None] = mapped_column(DateTime, nullable=True)
...
class BotControl(Base):
    ...
    portfolio_refresh_requested: Mapped[bool] = mapped_column(
...
class SymbolSnapshot(Base):
    ...
    signal: Mapped[str | None] = mapped_column(String(8), nullable=True)  # 'buy', 'sell', or None
    macd_hist: Mapped[float | None] = mapped_column(Float, nullable=True)
    bb_pct_b: Mapped[float | None] = mapped_column(
        Float, nullable=True
    )  # 0=lower band, 50=mid, 100=upper
...
class DesiredPosition(Base):
    ...
    target_usd: Mapped[float | None] = mapped_column(Float, nullable=True)  # buy amount
    signal_rsi: Mapped[float | None] = mapped_column(Float, nullable=True)
...
class Position(Base):
    ...
    recorded_at: Mapped[datetime] = mapped_column(DateTime, nullable=False)
    held_since: Mapped[datetime | None] = mapped_column(DateTime, nullable=True)
